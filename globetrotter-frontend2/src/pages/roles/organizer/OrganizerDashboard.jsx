// src/pages/roles/organizerdashboard.jsx
...
const OrganizerDashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setStats({
        ...
        totalRevenue: bookings.reduce((sum, booking) => sum + parseFloat(booking.total || 0), 0),
        averageRating: packages.reduce((sum, pkg) => sum + (pkg.rating || 0), 0) / (packages.length || 1)
      });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-gray-600 mb-8">Welcome back, {user?.first_name}! Here's your business overview.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="text-2xl font-bold text-purple-600">{format.currency(stats.totalRevenue)}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <button className="bg-green-600 text-white py-3 px-4 rounded-md hover:bg-green-700">
          Manage Bookings
        </button>
        <button className="bg-purple-600 text-white py-3 px-4 rounded-md hover:bg-purple-700">
          View Analytics
        </button>
      </div>
    </div>
